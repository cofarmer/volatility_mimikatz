...
# 4 for os version < win8
# -12 for os version == win8
g_adjust = 4 
...
class LsaDecryptor_Win8_x64(LsaDecryptor_x64):
  """Class for Windows 8 x64."""
  # MIMIKATZ x64: BYTE PTRN_WIN8_LsaInitializeProtectedMemory_KEY[]
  SIGNATURE	= '\x83\x64\x24\x30\x00\x44\x8b\x4d\xd8\x48\x8b\x0d'
  PTR_IV_OFFSET = 62;
  PTR_AES_KEY_OFFSET = 23;
  PTR_DES_KEY_OFFSET = -70;
  '''
  BCRYPT_HANDLE_KEY = construct.Struct('KIWI_BCRYPT_HANDLE_KEY',
      construct.Int32ul('size'),
...
class Wdigest_Win8_x64(Wdigest_x64):
  """Class for Windows 8 x64."""
  SIGNATURE	= '\x48\x3b\xd9\x74'
  FIRST_ENTRY_OFFSET = -4
...
class mimikatz(commands.Command):
  ...
  def init_objects(self, lsass_task):
    ...
    if len(self.profile) >= 7:
	  ...
	  global g_adjust
	  g_adjust = -12
          lsa_decryptor = LsaDecryptor_Win8_x64(lsass_task)
          wdigest = Wdigest_Win8_x64(lsass_task, self.credentials_obj)
      else:
        pass
